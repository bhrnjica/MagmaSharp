...
namespace LapackSharp
{
    unsafe public class LinAlg
    {
        ...
        // 
        [DllImport("LapackBinding.dll", CharSet = CharSet.Auto, SetLastError = true)]
        private static extern void  mbv2sgemm_cpu(bool rowmajor, int m, int n, int k, float alpha, float* A, int lda, float* B, int ldb, float beta, float* C, int ldc);
        [DllImport("LapackBinding.dll", CharSet = CharSet.Auto, SetLastError = true)]
        private static extern void mbv2dgemm_cpu(bool rowmajor, int m, int n, int k, double alpha, double* A, int lda, double* B, int ldb, double beta, double* C, int ldc);
        ...
        [DllImport("LapackBinding.dll", CharSet = CharSet.Auto, SetLastError = true)]
        private static extern int mbv2dinverse_cpu(bool rowmajor, int n, double* A, int lda);

        [DllImport("LapackBinding.dll", CharSet = CharSet.Auto, SetLastError = true)]
        private static extern int mbv2sinverse_cpu(bool rowmajor, int n, float* A, int lda);
        ...
        public static double[,] MInverse(double[,] A)
        {
            ...
            {
                //pInvoke call
                info = mbv2dinverse_cpu(true, n, pA, n);
