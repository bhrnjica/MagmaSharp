...
void mv2dgesv_test()
{
	...
	info = mbv2dgesv(n, nrhs, A, lda, ipiv, B, ldb);
...
void mv2dgesv_cpu_test()
{
	...
	/* Solve the equations A*X = B */
	info = mbv2dgesv_cpu(n, nrhs, A, lda, ipiv, B, ldb);
...
void mv2dgesv_gpu_test()
{
	const int n = 5;//the number of rows of A matrix
	const int nrhs = 3;//the number of columns of B matrix
	const int lda = n;//leadning dimension of A matrix is n (rows)
	const int ldb = n;//leading  dimensions of B matrix (same as A , rows)
	int ipiv[5];
	int info;
	...
	/* Solve the equations A*X = B */
	info = mbv2dgesv_gpu(n, nrhs, A, lda, ipiv, B, ldb);
...
void mv2sgesv_test()
{
	...
	/* Solve the equations A*X = B */
	info = mbv2sgesv(n, nrhs, A, lda, ipiv, B, ldb);
...
void mv2sgesv_cpu_test()
{
	...
	/* Solve the equations A*X = B */
	info = mbv2sgesv_cpu(n, nrhs, A, lda, ipiv, B, ldb);
...
void mv2sgesv_gpu_test()
{
	...
	/* Solve the equations A*X = B */
	info = mbv2sgesv_gpu(n, nrhs, A, lda, ipiv, B, ldb);
